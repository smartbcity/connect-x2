...
export const formatNumber = (num: number, language: string, fractionDigits: number = 2, isCurrency: boolean = false) => {
    const fractions = fractionDigits > 0 ? Math.pow(10, fractionDigits) : 1
    const fixedValue = Math.round(num * fractions) / fractions
    return new Intl.NumberFormat(language, { style: isCurrency ? 'currency' : "decimal", currency: 'USD' }).format(fixedValue)
}

export const distinct = <T>(array: T[], getKey: (item: T) => string | number = JSON.stringify) => {
    const seen = new Set()
